...
public class MessageConsumerService {
    ...
    public void handleBookingCreated(BookingMessage bookingMessage) {
        try {
            log.info("📨 Received booking created message: {}", bookingMessage.getBookingId());

            // Process booking confirmation logic
            log.info("🎫 Processing booking confirmation for: {}", bookingMessage.getUserEmail());
    ...
    public void handleEmailNotification(BookingMessage bookingMessage) {
        try {
            log.info("📧 Received email notification message: {}", bookingMessage.getBookingId());
    ...
    public void handleNotification(BookingMessage bookingMessage) {
        try {
            log.info("🔔 Received notification: {} for booking: {}",
                    bookingMessage.getMessageType(), bookingMessage.getBookingId());

            // Process notifications (push notifications, SMS, etc.)
            log.info("📲 Processing notification: {}", bookingMessage.getMessageType());
